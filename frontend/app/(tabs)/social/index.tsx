...
export default function Social() {
  ...
  const handleEditPhoto = async () => {
    ...
    if (!result.canceled) {
      try {
        setIsUploading(true);
        const newUrl = await uploadProfilePicture(result.assets[0].uri);
        if (newUrl) {
      ...
      <ScrollView className="flex-1" showsVerticalScrollIndicator={false}>
        ...
        <View className="items-center justify-center pt-8 pb-12 bg-black">
            <View className="shadow-lg shadow-golden/50">
                ...
                <TouchableOpacity 
                    ...
                    className="absolute bottom-0 right-0 bg-[#1a1a1a] p-2 rounded-full border border-golden/50"
                >
                    {isUploading ? (
        ...
        <View className="flex-1 bg-[#1a1a1a] rounded-t-[40px] px-6 pt-10 pb-20 min-h-screen">
            ...
            >
                ...
                <Text className="text-red-500 font-bold uppercase tracking-wider">Log Out</Text>
            </TouchableOpacity>
        </View>
      </ScrollView>
    </View>
